...
function Notificacoes({ user, C }: any) {
  ...
  return (
    <div className="space-y-3">
      ...
      {notifs.map(n => (
        <Card key={n.id} className="p-3 flex items-start gap-3" style={{ background: n.read ? C.cardBg : (C.cardBg + "ee"), borderColor: n.read ? C.border : "#ff5a1f" }}>
          ...
          <div className="flex-1">
            ...
            {n.link && <a href={n.link} target="_blank" rel="noreferrer" className="text-xs underline" style={{ color: "#ff5a1f" }}>Abrir</a>}
          </div>
